...
<div class="container modern-dashboard-wrapper">
    ...
    <div class="row mb-4 align-items-center">
        ...
        <div class="col-md-4 text-md-end">
            <button type="button" class="btn btn-dark rounded-pill px-4 shadow-sm" data-bs-toggle="modal" data-bs-target="#aiAssistantModal">
    ...
    <div class="row mt-4">
        ...
        <div class="col-lg-4 col-md-6 mb-4">
            <div class="card border-0 chart-card-premium h-100 p-4" style="background: linear-gradient(45deg, #6366f10d, #fff);">
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <h6 class="text-muted small fw-bold text-uppercase m-0">Visitor Traffic</h6>
                    <span class="badge bg-primary-subtle text-primary" style="font-size: 0.6rem;">Activity Log</span>
                </div>
                <div class="d-flex align-items-end justify-content-between">
                    <h3 class="fw-bold text-dark mb-0"><?= array_sum($monthlyVisitors) ?></h3>
                    <div style="width: 100px; height: 45px;">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            <div class="modal-header border-0 pb-0">
                <h5 class="fw-bold"><i class="bi bi-robot text-primary me-2"></i>AI Content Writer</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    <label class="small fw-bold text-muted">What do you want to write?</label>
                    <textarea id="aiPrompt" class="form-control rounded-3" rows="3" placeholder="e.g. Write a notice for 2 hours electricity maintenance on Sunday"></textarea>
                </div>
                <div class="d-grid">
                    <button type="button" id="generateAI" class="btn btn-primary rounded-pill">
                        <span id="aiBtnText">Generate Content</span>
                        <span id="aiLoader" class="spinner-border spinner-border-sm d-none"></span>
                    </button>
                </div>
                <div id="aiResultContainer" class="mt-4 d-none">
                    <label class="small fw-bold text-success">Generated Draft:</label>
                    <div id="aiResult" class="p-3 bg-light rounded-3 border small" style="white-space: pre-wrap;"></div>
                    <button class="btn btn-sm btn-outline-secondary mt-2 w-100 rounded-pill" onclick="copyAIContent()">Copy to Clipboard</button>
                </div>
            </div>
        </div>
    </div>
...
document.getElementById('generateAI')?.addEventListener('click', async function() {
    ...
    try {
        // Note: You will need to create a simple PHP proxy to call Google Gemini or OpenAI API
        const response = await fetch('core/ai_handler.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ prompt: prompt })
        });
        const data = await response.json();
        resultDiv.innerText = data.text || "AI could not generate content. Check API key.";
        resultContainer.classList.remove('d-none');
    } catch (e) {
        resultDiv.innerText = "Error: AI Service Unavailable.";
        resultContainer.classList.remove('d-none');
    } finally {
